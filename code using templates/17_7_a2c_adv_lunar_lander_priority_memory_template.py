...
class A2CAgent:
    def __init__(self, state_size, action_size):
        ...
        self.a_model = ModelActor(
            self.state_size, self.action_size, args.hidden_size).to(self.device)
        self.c_model = ModelCritic(
            self.state_size, self.action_size, args.hidden_size).to(self.device)
    ...
    def replay(self):
        ...
        v_t = self.c_model(s_t0).squeeze()
        A = td_delta-v_t

        a_all = self.a_model(s_t0)
...
for e in range(args.episodes):
    ...
    # TODO
    for t, (st0, at0, rt1) in enumerate(transitions):
        if t < len(transitions)-1:
            st1, _, _ = transitions[t+1]
            st1 = torch.FloatTensor([st1]).to(args.device)
            vt = agent.c_model(st1).squeeze().cpu().item()
            rt1 += args.gamma*vt
        agent.replay_memory.push((st0, at0, rt1))
    e_losses = []
