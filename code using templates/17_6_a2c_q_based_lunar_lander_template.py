...
class A2CAgent:
    ...
    def replay(self):
        ...
        s_t0, a_t0, s_t1, is_end, r_t1 = zip(*batch)
...
for e in range(args.episodes):
    ...
    for t in range(args.max_steps):
        ...
        agent.replay_memory.push([s_t0, a_t0, s_t1, is_end, r_t1])
        s_t0 = s_t1
    ...
    e_losses = []
    for _ in range(int(1.5 * np.ceil(t/args.batch_size))):
    ...
    if e % 100 == 0:
        ...
        plt.subplot(3, 1, 1)
        plt.ylabel('Score')
        plt.plot([max(it, 0) for it in all_scores])
