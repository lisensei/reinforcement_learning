...
parser.add_argument('-is_render', default=('COLAB_GPU' not in os.environ),
...
parser.add_argument("-target_update", default=2000, type=int)
...
class PGAgent:
    def __init__(self, state_size, action_size):
        ...
        self.p_model = Model(self.state_size, self.action_size,
        ...
        self.replay_memory = ReplayPriorityMemory(
            args.replay_buffer_size, args.batch_size)

    def update_target(self,):
        self.target.load_state_dict(self.p_model.state_dict())
    ...
    def replay(self):
        ...
        # TODO
        a_all = self.p_model(s_t0)
        dist = distributions.Categorical(a_all)
        a_t = dist.log_prob(a_t0)

        a_all = self.p_model(s_t0)
        a_target = dist.log_prob(a_t0)
        r_c = (r_c-np.mean(self.replay_memory.Rs)) / \
            np.std(self.replay_memory.Rs)
        loss = -r_c*(a_t+1e-8)/(a_target+1e-8)
...
for e in range(args.episodes):
    ...
    for t, transition in enumerate(transitions):
        ...
        for tf, transition_f in enumerate(transitions[t:]):
            _, _, rf = transition_f
            Rt += args.gamma**tf*rf
        agent.replay_memory.push((st, at, Rt))

    e_losses = []
    for _ in range(int(1.5*np.ceil(1.5*len(transitions)/args.batch_size))):
