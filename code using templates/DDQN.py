...
parser.add_argument('-is_render', default=True,
...
class DQNAgent:
    def __init__(self, state_size, action_size):
        ...
        self.q_model = Model(self.state_size, self.action_size,
                             args.hidden_size).to(self.device)

        self.target_model = Model(self.state_size, self.action_size,
                                  args.hidden_size).to(self.device)
    ...
    def replay(self):
        ...
        batch, replay_indices = self.replay_memory.sample()
        ...
        is_not_end = torch.FloatTensor(
            (np.array(is_end) == False) * 1.0).to(args.device)

        # TODO
        INDICES = torch.arange(args.batch_size).to(args.device)
        q_t0_all = self.q_model(s_t0)
        q_t0 = q_t0_all[INDICES, a_t0-1]
        q_t1_all = self.target_model(s_t1).detach()
        a_t1 = q_t1_all.argmax(dim=1)
        q_t1 = q_t1_all[INDICES, a_t1]

        q_t1_final = r_t1+is_not_end * (args.gamma*q_t1)

        td_error = (q_t0-q_t1_final)**2
        self.replay_memory.update_priorities(replay_indices, td_error)
